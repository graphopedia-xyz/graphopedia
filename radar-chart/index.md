...
is a graphical method of showing multivariate data with multiple variables on radial axes.[^friendly] The minimum number is three variables because the simplest shape made by the radar chart is a triangle. Each radial axis represents one variable which can have its own scale. The variables are independent from each other. All the axes are positioned at an equal angle.
...
Functionally, the radar chart is the same as [parallel coordinates](/parallel-coordiantes) except set in polar coordinates.
